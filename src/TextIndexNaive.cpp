...
public:
    ...
    public:
                   ...
                   size_t                                           p_length )
        {
            ...
            bool found = false;
            for ( auto const& use: m_body)
            {
                if ( use.ObjectId() == p_object )
                {
                    found = true;
                    break;
                }
...
public:
    ...
    const Node* Locate ( const char32_t* p_term ) const
    {
        return m_root.Find ( p_term );
    }
...
TextIndexNaiveReader :: Locate ( const char32_t* p_term ) const
{
    assert ( m_index );
